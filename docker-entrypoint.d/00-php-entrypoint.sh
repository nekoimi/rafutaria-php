...
exec_config_php_ini() {
  ...
  if [ -f "$PHP_INI_DIR/php.ini-production" ]; then
    ...
    # 生产环境关闭 validate_timestamps 以提升性能
...
exec_config_xdebug() {
  ...
  if [ "$PHP_XDEBUG_ENABLE" = "true" ]; then
    ...
    XDEBUG_VERSION=$(php -r "echo phpversion('xdebug');" 2>/dev/null | cut -d. -f1)
    ...
    if [ "$XDEBUG_VERSION" = "2" ]; then
      ...
      echo "WARNING: Unrecognized xdebug version: '$XDEBUG_VERSION'. Disabling xdebug."
      rm -f "$DOCKER_PHP_EXT_XDEBUG"
    fi
...
exec_config_composer() {
  ...
  if [ -f "$WORKSPACE/composer.lock" ]; then
    ...
    composer dump-autoload --optimize --working-dir="$WORKSPACE"
